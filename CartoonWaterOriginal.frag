...
float getLayerColorFactor(float distance) {
    ...
    if(distance > layer2Distance) { 
        factor -= layerDarkenFactor;
    }
    if(distance > layer3Distance) { 
        factor -= layerDarkenFactor;
    }        
    if(distance > layer4Distance) { 
        factor -= layerDarkenFactor;
    }        
    if(distance > layer5Distance) { 
        factor -= layerDarkenFactor;
    }        
    
    if(distance < layer1Distance) {
        factor = 1.3;
    }
    return factor;
}
...
void main(void)
{
    ...
    //float a = (sinus3 + sinus3Y) *-1.0;
    //appliedColor.xyz *= getLayerColorFactor(patternHeight - patternRelativeY + a);

    if(patternRelativeY < sinus3 + patternHeight) {
        // is in wave 2
        appliedColor = color3;
        appliedColor.xyz *= getLayerColorFactor(sinus3 + patternHeight - patternRelativeY);
    }
    
    if(patternRelativeY < sinus1) {
        // is in wave 3
        appliedColor = color1;
        appliedColor.xyz *= getLayerColorFactor(sinus1 - patternRelativeY);
    }
    
    if(patternRelativeY < sinus2) {
        // is in wave 4
        appliedColor = color2;
        appliedColor.xyz *= getLayerColorFactor(sinus2 - patternRelativeY);
    }
    
    if(patternRelativeY < sinus3) {
        // is in wave 5
        appliedColor = color3;
        appliedColor.xyz *= getLayerColorFactor(sinus3 - patternRelativeY);
    }
    
    //if(isOddpattern) appliedColor = vec3(0.0,0.0,0.0);
    vec4 alphaColor = vec4(appliedColor.x, appliedColor.y, appliedColor.z, 1.0);
